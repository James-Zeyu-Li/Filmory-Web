...
* **后端 (Back-End)**
  ...
  - [ ] **存储抽象层服务契约**：设计 [IStorageService.ts](file:///Users/james/Desktop/1.2-CS/00-projects/Filmory-Web/backend/src/services/IStorageService.ts) 接口 (DIP 原则)。
  - [ ] **本地磁盘存储服务实现**：实现 `LocalDiskStorageService` (写入本地 uploads 目录并配置 Express 静态资源服务)。
  - [ ] **MinIO/S3 兼容对象存储实现**：实现 `S3StorageService` (连接本地 MinIO 容器 / 生产级 AWS S3)。
