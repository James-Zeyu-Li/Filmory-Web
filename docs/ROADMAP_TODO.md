...
- 核心产品：Dashboard、照片库、拍摄卷/项目集、器材库、财务、统计、对比、标签、Excel 导入/导出已覆盖当前胶片摄影工作流；统计已去掉无意义总照片数，Dashboard 已改为胶片用户优先。
...
- 当前验证状态：最近一轮 `npm run lint`、`npm run test`、`npm run build`、`npx playwright test e2e/roll-cover-refresh.spec.ts e2e/film-backs.spec.ts` 均通过；默认 Vitest 为 `23 passed / 3 skipped`、`71 passed / 5 skipped`；构建仅有 Vite chunk size warning，lint 当前零 warning。

## Next Up

1. [ ] **真实 App 开启 Supabase Sync smoke**
...
3. [ ] **危险操作取消态 E2E 补强**
   - 在现有 `ConfirmContext` 已接入的基础上，补充取消删除、取消账号注销、取消重置等黑盒测试，确认取消路径不会产生数据变更。
...
- [ ] **PWA 更新提示**
  - 当新 Service Worker 发布时，给用户明确的“更新到新版本”提示，避免长期停留在旧缓存。

## P2：体验与功能优化

- [ ] **表单与微交互一致性复核**
  - 系统检查 Settings、Gear、Rolls、Photos、Finance 的输入 focus 状态、二级 icon button hover、destructive hover、窗口 resize/reflow 和弹窗内容折叠行为。
...
- [ ] **胶卷分享卡片生成器**
  - 目标：为单张照片或一卷精选照片生成可导出的复古胶片/拍立得风格分享卡片，方便发布到小红书、Instagram、朋友圈等渠道。
  - 内容：卡片可包含照片、相机、镜头、胶卷品牌/型号、ISO、光圈、快门、焦段、拍摄日期、地点、评分和简短备注；缺失字段应自动隐藏。
  - 交互：从照片详情、拍摄卷详情或精选照片入口打开；提供 2 到 3 个模板、预览、导出 PNG/JPEG 和复制/下载动作。
  - 测试要求：补导出数据映射单测，至少一条 Playwright smoke 覆盖打开生成器、选择模板、生成预览和下载按钮可用。

- [ ] **i18n 国际化**
  - 目标：支持用户在 Settings 中切换界面语言，首期支持 `zh-CN` 与 `en-US`；语言偏好持久化到本地 workspace preference，后续接 Supabase 时同步到 `user_profiles`。
  - 技术路径：配置 `i18next` / `react-i18next`，建立 `frontend/src/i18n/locales/zh-CN.json` 与 `en-US.json`，优先抽离导航、Landing、Auth、Settings、Dashboard 和核心表单/反馈文案。
  - 范围边界：用户自己输入的数据、相机/镜头/胶卷品牌型号、EXIF 参数和历史记录内容不自动翻译；reference catalog 名称保持原始名称。
  - 格式化：日期、数字、货币显示通过 `Intl` 或统一 formatter 根据当前语言/货币偏好输出；不要把货币换算和语言切换绑定。
  - 测试要求：补语言偏好持久化 unit test，补至少一条 Playwright smoke 覆盖 Settings 切换语言后导航/Auth/Dashboard 关键文案变化，刷新后仍保持选择。
