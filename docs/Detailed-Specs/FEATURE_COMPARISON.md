...
| 模块 | Web 当前状态 | 待处理 |
| :--- | :--- | :--- |
| 认证 | 已实现登录/注册共页、未验证邮箱重发、找回密码、重设密码、认证回跳、验证状态页、密码策略提示、安全回跳兜底和工作区固定 Account Center 入口；本地 Mailpit 邮件链路已验证 | 生产 SMTP、OAuth provider 和线上 Redirect URL 配置验证 |
...
| 照片与相册 | 全局照片库/相册已从主导航和路由入口隐藏；底层照片资产仍用于拍摄卷封面、样片 fallback 和历史数据兼容 | 后续决定迁移到拍摄卷内样片能力，或删除 legacy UI 文件 |
