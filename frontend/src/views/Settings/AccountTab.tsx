...
export const AccountTab: React.FC<AccountTabProps> = ({ onCloseSettings }) => {
  ...
  // Navigating away from the current route is itself what closes Settings
  // (it's rendered from a `?settings=` param on whatever page is current —
  // see App.tsx), so these must not also call onCloseSettings() first: that
  // can trigger an async navigate(-1) that races with — and can clobber —
  // the navigate() below, landing back on the page Settings was opened from
    ...
    <div className="account-tab">
      ...
      {(isTrial || !user) && (
        <div className="account-trial-actions">
          ...
          {isTrial && (
            <button type="button" className="account-text-button" onClick={onCloseSettings}>
              {t('account.keepTrial')}
            </button>
          )}
        </div>
