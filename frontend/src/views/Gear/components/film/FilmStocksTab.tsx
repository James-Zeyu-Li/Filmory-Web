...
import { Film, Minus, Plus, Search, Trash2, Upload } from 'lucide-react';
  ...
  return (
    <div className="lenses-grid-layout">
      {displayFilms.map(film => {
        ...
        return (
          <article key={film.id} className="gear-card lens-card-horizontal film-stock-card" onClick={() => onEdit(film)}>
            <button
              type="button"
              className="gear-card-open-action"
              onClick={event => { event.stopPropagation(); onEdit(film); }}
              aria-label={`${t('gear.editFilmStock')}: ${film.name}`}
            />
