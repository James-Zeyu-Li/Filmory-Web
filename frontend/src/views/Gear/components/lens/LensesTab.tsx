...
import { Archive, Edit2, Plus, Search, SlidersHorizontal, Trash2, Upload } from 'lucide-react';
...
interface LensesTabProps {
  ...
  onAdd: () => void; onView: (lens: Lens) => void; onEdit: (lens: Lens) => void; onDelete: (id: string) => void; onArchive: (lens: Lens) => void; onUpload: (id: string) => void; onPreview: (url: string) => void;
}
const getAvatarUrl = (url?: string | null) => url && (url.startsWith('http') || url.startsWith('data:') || url.startsWith('blob:')) ? url : null;

export const LensesTab = memo(({ lenses, searchQuery, sortBy, t, uploadingEntityId, onAdd, onView, onEdit, onDelete, onArchive, onUpload, onPreview }: LensesTabProps) => {
  ...
  return <div className="lenses-grid-layout">{displayLenses.map(lens => {
    ...
    return <div key={lens.id} className="gear-card gear-row-card" onClick={() => onView(lens)} style={{ cursor: 'pointer' }}>
      <button type="button" className="gear-card-open-action" onClick={event => { event.stopPropagation(); onView(lens); }} aria-label={t('gear.viewLensHistory', { name: lens.name })} />
      ...
      <div className="lens-card-content"><div className="gear-card-header"><span className={`tag lens-${lens.type || 'prime'}`}>{lens.type === 'prime' ? t('gear.prime') : t('gear.zoom')}</span><div style={{ display: 'flex', gap: '8px' }}><IconButton icon={<Edit2 size={16} />} title={t('gear.editLens')} onClick={event => { event.stopPropagation(); onEdit(lens); }} /><IconButton variant="success" icon={<Archive size={16} />} title={t('gear.sellArchive')} onClick={event => { event.stopPropagation(); onArchive(lens); }} /><IconButton variant="danger" icon={<Trash2 size={16} />} title={t('gear.deletePermanently')} onClick={event => { event.stopPropagation(); onDelete(lens.id!); }} /></div></div><h3>{lens.name}</h3><div className="gear-details"><div><strong>{t('gear.focalLength')}:</strong> {lens.focalLength}mm</div><div><strong>{t('gear.maxAperture')}:</strong> {lens.maxAperture}</div></div></div>
    </div>;
  })}</div>;
});
