...
export const LensesTab = memo(({ lenses, searchQuery, sortBy, t, uploadingEntityId, onAdd, onEdit, onDelete, onArchive, onUpload, onPreview }: LensesTabProps) => {
  ...
  return <div className="lenses-grid-layout">{displayLenses.map(lens => {
    ...
    return <div key={lens.id} className="gear-card lens-card-horizontal" onClick={() => onEdit(lens)} style={{ cursor: 'pointer' }}>
      ...
      <div className="lens-card-content"><div className="gear-card-header"><span className={`tag lens-${lens.type || 'prime'}`}>{lens.type === 'prime' ? t('gear.prime') : t('gear.zoom')}</span><div style={{ display: 'flex', gap: '4px' }}><IconButton variant="success" icon={<Archive size={16} />} title={t('gear.sellArchive')} onClick={event => { event.stopPropagation(); onArchive(lens); }} /><IconButton variant="danger" icon={<Trash2 size={16} />} title={t('gear.deletePermanently')} onClick={event => { event.stopPropagation(); onDelete(lens.id!); }} /></div></div><h3>{lens.name}</h3><div className="gear-details"><div><strong>{t('gear.focalLength')}:</strong> {lens.focalLength}mm</div><div><strong>{t('gear.maxAperture')}:</strong> {lens.maxAperture}</div></div></div>
    </div>;
  })}</div>;
});
